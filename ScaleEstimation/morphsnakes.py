...
def morphological_chan_vese(image, iterations, init_level_set='checkerboard',
    ...
    for _ in range(iterations):
        ...
        # inside = u > 0
        # outside = u <= 0
        c0 = (image * (1 - u)).sum() / float((1 - u).sum() + 1e-8)  #doutside mean
        c1 = (image * u).sum() / float(u.sum() + 1e-8)              #inside mean
...
def morphological_chan_vese_mask(image, iterations, mask, init_level_set='checkerboard',
                            smoothing=1, lambda1=1, lambda2=1,
                            iter_callback=lambda x: None):
    ...
    for _ in range(iterations):
        ...
        # inside = u > 0
        # outside = u <= 0
        c0 = (image * mask * (1 - u)).sum() / float((mask * (1 - u)).sum() + 1e-8)  #doutside mean
        c1 = (image * mask * u).sum() / float((mask * u).sum() + 1e-8)              #inside mean

        # Image attachment
        du = mask * np.gradient(u)
        abs_du = np.abs(du).sum(0)
        aux = abs_du * (lambda1 * (image - c1)**2 - lambda2 * (image - c0)**2)
...
def morphological_geodesic_active_contour(gimage, iterations,
    ...
    for _ in range(iterations):
        ...
        # Image attachment
        aux = np.zeros_like(image).astype(float)
        du = np.gradient(u)
